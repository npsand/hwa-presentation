...
parser.add_argument('-sdai', "--skip_depthai", action="store_true",
                    help="Skip installation of depthai library.")
parser.add_argument('-dr', "--dry_run", action="store_true",
                    help="Print commands without executing.")
parser.add_argument("--convert", nargs="?", default=convert_default,
                    help="Convert the NN blobs using BlobConverter. Can be used as --convert 2021.4 to convert using OpenVINO 2021.4 or just --convert to use latest OpenVINO release")
...
in_venv = getattr(sys, "real_prefix", getattr(
    sys, "base_prefix", sys.prefix)) != sys.prefix
...
if sys.version_info[0] != 3:
    raise RuntimeError(
        "Examples require Python 3 to run (detected: Python {})".format(sys.version_info[0]))
...
if not args.skip_depthai:
    ...
    if not git_context or git_branch == 'main':
        ...
        depthai_install_cmd = [*pip_package_install,
                               '-U', '--force-reinstall', 'depthai']
        if args.dry_run:
...
if args.convert != convert_default:
    ...
    blobconverter_cmds = [
        [sys.executable, "-m", "blobconverter", "-zn", nn_name, "-sh",
            str(nn_shave), "-o", f"{examples_dir}/models", *(["-v", args.convert] if args.convert is not None else [])]
